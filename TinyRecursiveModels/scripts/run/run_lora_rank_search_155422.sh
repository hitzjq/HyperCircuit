...
# ==========================================
# TRM LoRA Rank Search Script (Checkpoint 155422)
# 改动：改一下datapath和ckpt就行，这个ckpt用155422
# ==========================================
...
for r in "${RANKS[@]}"; do
    ...
    run_name="LoRA_r${r}_ckpt155422_$(date +%m%d_%H%M)"
    LOG_DIR="checkpoints/${run_name}"
    mkdir -p "${LOG_DIR}"

    echo "=========================================================="
    echo "Starting training for Rank ${r} (Checkpoint 155422)"
    ...
    torchrun --nproc-per-node=8 --rdzv_backend=c10d --rdzv_endpoint=localhost:0 --nnodes=1 lora_finetune.py \
        ...
        +load_checkpoint=checkpoints/ARC-AGI-1/step_155422 \
        lora_r=${r} \
        lora_alpha=${alpha} \
        lr=1e-4 \
        +lora_dropout=0.05 \
...
echo "所有 Rank 测试完毕 (Checkpoint 155422)。"
