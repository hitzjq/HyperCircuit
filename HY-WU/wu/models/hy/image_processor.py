...
from .tokenization_hunyuan_image_3 import CondImage, ImageInfo, ImageTensor, Resolution, ResolutionGroup
...
class HunyuanImage3ImageProcessor(object):
    def __init__(self, config):
        ...
        self.vae_info = VAEInfo(
            ...
            down_h_factor=config.vae_downsample_factor[0],
            down_w_factor=config.vae_downsample_factor[0],
    ...
    def vit_process_image(self, image) -> ImageTensor:
        ...
        return self.as_image_tensor(
            image, image_type=self.vit_info.image_type, origin_size=origin_size, **remain_kwargs
        )
    ...
    def prepare_full_attn_slices(self, output, batch_idx=None, with_gen=True):
        ...
        if self.cond_image_type == "vae":
            cond_choices = dict(
                causal=[], full=output.vae_image_slices[batch_idx] if batch_idx is not None else output.vae_image_slices
            )

        elif self.cond_image_type == "vit":
            cond_choices = dict(
                causal=[], full=output.vit_image_slices[batch_idx] if batch_idx is not None else output.vit_image_slices
            )
            ...
            for batch_index, (output_image, cond_images) in enumerate(zip(outputs, batch_cond_images)):
                ...
                if len(cond_images) == 0:
                    ...
                    for cond_image_ratio_index, cond_image_ori_width, cond_image_ori_height in zip(
                        cond_images_ratio_index_list, cond_images_ori_width_list, cond_images_ori_height_list
                    ):
...
__all__ = ["HunyuanImage3ImageProcessor"]
