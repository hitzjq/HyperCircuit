...
def load_weights(model, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
    def update_state_dict(state_dict: dict[str, torch.Tensor], name, weight):
        ...
        if isinstance(weight, torch.Tensor):
            ...
            raise ValueError(f"Unsupported tensor type in load_weights " f"for {name}: {type(weight)}")
...
def _worker(path, config, rank=None, world_size=None, port=None, req_queue=None, rsp_queue=None):
    """
    each rank's worker:
    ...
    load_weights(vae, merged_state_dict)
    vae = vae.cuda()
