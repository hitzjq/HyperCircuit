...
def create_model(config: PretrainConfig, train_metadata: PuzzleDatasetMetadata, rank: int, world_size: int):
    ...
    with torch.device("cuda"):
        ...
        # Build module_specs for Parameter Generator
        module_specs = []
        for name, layer in lora_dict_refs.items():
    ...
    return bundle, optimizers, optimizer_lrs
...
def init_train_state(config: PretrainConfig, train_metadata: PuzzleDatasetMetadata, rank: int, world_size: int):
    ...
    # Save TRM (which includes the embedded embeddings)
    torch.save(train_state.model["trm"].state_dict(), os.path.join(config.checkpoint_path, f"step_{train_state.step}"))
    # Save PG Model
    torch.save(train_state.model["pg"].state_dict(), os.path.join(config.checkpoint_path, f"step_{train_state.step}_pg"))
...
def train_batch(config: PretrainConfig, train_state: TrainState, batch: Any, global_batch_size: int, rank: int, world_size: int):
    ...
    if train_state.carry is None:
        with torch.device("cuda"):
            train_state.carry = train_state.model["trm"].initial_carry(batch)  # type: ignore

    # >>> TWO-PASS FORWARD START <<<
    trm_model = train_state.model["trm"]
    pg_model = train_state.model["pg"]
    ...
    for name, layer in lora_refs.items():
        lora_A = lora_dict[f"{name}.lora_A"]
        lora_B = lora_dict[f"{name}.lora_B"]
        layer.set_dynamic_lora(lora_A, lora_B, scale=lora_dict["scale"])
        
    # Pass 3: Actual Inference & Reasoning
    train_state.carry, loss, metrics, _, _ = trm_model(carry=train_state.carry, batch=batch, return_keys=[])
    
    # Detach LoRA layers heavily to prevent inter-batch memory leaks
    for layer in lora_refs.values():
        layer.set_dynamic_lora(None, None, None)
    # >>> TWO-PASS FORWARD END <<<
