...
# 格式: "标签|pg_num_blocks|pg_d_model|lora_r|lora_alpha|weight_decay|lr|puzzle_lr|condition_mode"
EXPERIMENTS=(
    "E2_wd03|2|256|16|32|0.3|1e-5|1e-3|full_trm"
    "F1_emb_only|2|256|16|32|0.1|1e-5|1e-3|embedding_only"
)

for EXP in "${EXPERIMENTS[@]}"; do
    IFS='|' read -r TAG PG_BLOCKS PG_DIM LORA_R LORA_ALPHA WD LR PUZZLE_LR COND_MODE <<< "$EXP"
    run_name="WU4TRM_${TAG}_bs${GLOBAL_BATCH_SIZE}_${NUM_GPUS}gpus_$(date +%m%d_%H%M)"
    CKPT_DIR="checkpoints/${run_name}"
    mkdir -p "${CKPT_DIR}"
    echo "Running: ${TAG} | pgb=${PG_BLOCKS} pgd=${PG_DIM} r=${LORA_R} a=${LORA_ALPHA} wd=${WD} lr=${LR} plr=${PUZZLE_LR} cond=${COND_MODE}"
    torchrun --nproc-per-node=${NUM_GPUS} --rdzv_backend=c10d --rdzv_endpoint=localhost:0 --nnodes=1 \
        ...
        condition_mode="${COND_MODE}" pg_num_blocks=$PG_BLOCKS pg_d_model=$PG_DIM \
