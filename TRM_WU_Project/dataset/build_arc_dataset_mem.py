...
def convert_dataset(config: DataProcessConfig):
    ...
    for split_name, split in data.items():
        ...
        total_puzzles_count = 0
        total_groups_count = 0
        
        for subset_name, subset in split.items(): # "all" is the only subset
            # Pass 1: Count total examples to pre-allocate memmap
            total_examples_for_subset = 0
            for group in subset:
                for puzzle in group:
                    total_examples_for_subset += len(puzzle.examples)
            
            # Pre-allocate using open_memmap
            # This creates a valid .npy file on disk immediately.
            inputs_path = os.path.join(split_dir, f"{subset_name}__inputs.npy")
            labels_path = os.path.join(split_dir, f"{subset_name}__labels.npy")
            
            inputs_mm = np.lib.format.open_memmap(inputs_path, mode='w+', dtype=np.uint8, shape=(total_examples_for_subset, ARCMaxGridSize * ARCMaxGridSize))
            labels_mm = np.lib.format.open_memmap(labels_path, mode='w+', dtype=np.uint8, shape=(total_examples_for_subset, ARCMaxGridSize * ARCMaxGridSize))

            results = {k: [] for k in ["puzzle_identifiers", "puzzle_indices", "group_indices"]}
            ...
            for group in subset:
                for puzzle in group:
                    ...
                    for _idx_ex, (inp, out) in enumerate(puzzle.examples):
                        ...
                        # Write directly to memmap instead of appending to a massive list
                        inputs_mm[example_id] = inp
                        labels_mm[example_id] = out
                        example_id += 1
            ...
            # Save the integer-based metadata arrays
            for k, v in results.items():
                v = np.array(v, dtype=np.int32)
                np.save(os.path.join(split_dir, f"{subset_name}__{k}.npy"), v)
        ...
        metadata = PuzzleDatasetMetadata(
            ...
            mean_puzzle_examples=total_examples_count / total_puzzles_count,
